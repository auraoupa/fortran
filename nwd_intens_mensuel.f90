PROGRAM nwd_intens_mensuel
  ...
  narg=iargc()
  IF ( narg == 0 ) THEN
     PRINT *,' USAGE : nwd_intens_mensuel -f filein -o fileout -p fileout2'
     PRINT *,'         '
     STOP
  ENDIF
  ...
  DEALLOCATE( precipin, precipout, intens )
...
END PROGRAM nwd_intens_mensuel
